...
Created by frederative to fulfill the gif needs of the forge.
"""
...
class DaVinciClient(discord.Client):
  ...
  async def on_message(self, message):
    ...
    if message.content == "!help":
      ...
      desc += "Message frederative for additional feature requests"
      embed = discord.Embed(title="DaVinci Entity Help", description=desc, color=discord.Color.blue())
    ...
    #  await message.channel.send("TBD ಠ_ಠ")
      ...
      for r in message.author.roles:
        if r.name == 'Founder Mods' or r.name == 'Overlords':
          isAdmin = True
          break
      ...
      if isAdmin:
        ...
        if len(_pages) > 0:
            ...
            embed = discord.Embed(title="GIFS page 1", description=content, color=discord.Color.blue())
            msg = await message.channel.send(embed=embed)#_resp)
      ...
      # Only admins allowed
      if isAdmin:
        channel_id = 8675309 # replace with gif channel
...
#          try:
            ...
            await message.channel.send("Hack the planet: {0}.".format(_title))
            print(_resp)
#          except:
...
client.run(os.getenv('THE_TOKEN'))
